...
export default class InGameConsumer extends BaseInGameConsumer {
  ...
  /**
   * @param {import("fish-prawn-crab-pro").Player} realPlayer
  ...
  async publishSignedInPlayer(realPlayer) {
    ...
    const payload = {
      player: {
        ...
        red: user.red,
        type: user.type,
        currency: realPlayer.session.currency,
      },
    };
  ...
  storePlayerInGame(realPlayer, user) {
    user &&
      (this.gameData.players[realPlayer.UID] = Object.assign({}, user, { id: realPlayer.UID, currency: realPlayer.session.currency }));
  }
}
