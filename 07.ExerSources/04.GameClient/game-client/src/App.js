...
listenFromAPI() {
  ...
  socket.on("UpdateLastRoundResult", data => {
    if(data == null) {
      return;
    }
    ...
    this.setState({ 
      ...
      lastResult: data.betOptions.split(","),
  ...
  render() {
      ...
      <div>
        ...
        <div id="game-container" className={"game-container " + (this.state.joinedUsername ? "" : " hide")}>
          ...
          <div className="group last-round-group">
              <div>Last Round: {this.state.lastRoundId}</div>
              <div id="last-result" className="last-result">
                  {
                      this.state.lastResult.map(b =>
                          <div className={"bet-option bet-" + b}
                              style={{backgroundImage: 'url("/images/' + b +'.png")'}}></div>)
                  }
              </div>
              <div>Your bets on last round {this.state.lastRoundId}:</div>
              <div id="last-bets" className="last-bets">
                  {
                      this.state.lastBets.map(b =>
                          <div className={"bet-option bet-" + b}
                              style={{backgroundImage: 'url("/images/' + b +'.png")'}}></div>)
                  }
              </div>
